...
genai.configure(api_key="YOUR GEMINI API KEY")   #your gemini api key 
generation_config = {
    "temperature": 0.7,
    "top_p": 0.95,
    "top_k": 40,
        ...
        for idx, row in shopping_df.iterrows():
            ...
            with st.container(border=True):
                ...
                with cols[2]:
                    ...
                    expiry_date_key = f"expiry_date_{item_name.replace(' ', '_')}_{idx}"
                    buy_button_key = f"buy_button_{item_name.replace(' ', '_')}_{idx}"
                    remove_button_key = f"remove_button_{item_name.replace(' ', '_')}_{idx}"
...
def main():
    ...
    nav_options = {
        ...
        "Leftover Transformation": "♻️ Leftover Transformation", # New navigation option
